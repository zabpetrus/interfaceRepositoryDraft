...
const CppSection2D = (props: Props) => {  
     ...
     */
    var phase = useSelector((state: AppState | undefined ) => state?.phase);
    var desvio = useSelector((state: AppState | undefined ) => state?.desvio);
    const numero_de_fases = useSelector((state: AppState | undefined ) => state?.numPhases);

    //Mudança de tipo para iterar no loop. Garando que o numero de fases é um número, não outra coisa
     var numPhases: number = 0;

    if (undefined !== numero_de_fases){
        numPhases =  numero_de_fases;
    }   
   ...
   //As útimas paredes. Elas são colocadas separadamente
    ...
    for(i; i < numPhases; i++)
    {
        ...
        var od = validaPhase(phase, i, 'od');
        var label_od = labelOD(od);
                      
        /**
         * Valores iniciais para a fase
         * Serão obtidos durante iteração do valor da fase
         */
        ...
        AppTextLoader(scene, label_od, od_label_size, od_label_xpos, od_label_ypos );
        ...
        if(i === 1){
            ...
            var paredecimentada = clonarobjetoeposicaosimetria( paredealterada, fundo, deslocamentox, -pattern, 0.001);
        ...
        size -= pattern;
        od_label_ypos = od_label_ypos - label_ypos_desl; //A posição y desce uma unidade porque a altura da fase é estabelecida como sendo uma unidade 
